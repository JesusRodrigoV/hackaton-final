...
class Desembolso(Base):
    ...
    estado = Column(SAEnum(DesembolsoEstado, name="desembolso_estado", native_enum=False), nullable=False, default=DesembolsoEstado.PROCESANDO)
...
class PlanPagosCobranza(Base):
    ...
    monto = Column(Numeric(14, 2), nullable=False)
    estado_pago = Column(SAEnum(PagoEstado, name="pago_estado", native_enum=False), nullable=False, default=PagoEstado.AL_DIA)
import uuid
from decimal import Decimal
from sqlalchemy import Column, String, Integer, Text, Numeric, ForeignKey, Enum
from sqlalchemy.dialects.postgresql import UUID as PG_UUID
from sqlalchemy.orm import relationship
...
class DesembolsoEstado(enum.Enum):
    ...
    billetera_id = Column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    usuario_id = Column(PG_UUID(as_uuid=True), ForeignKey("usuarios.usuario_id"), nullable=False)
    tipo = Column(Enum(BilleteraTipo, values_callable=lambda obj: [e for e in obj.__dict__ if not e.startswith("__")], name="billetera_tipo"), nullable=False)
class PagoEstado(enum.Enum):

    usuario = relationship("Usuario", backref="billeteras")


class DesembolsoEstado(str):
    PROCESANDO = "PROCESANDO"
    COMPLETADO = "COMPLETADO"
    FALLIDO = "FALLIDO"
...
class Desembolso(Base):
    ...
    billetera_id = Column(PG_UUID(as_uuid=True), ForeignKey("billeteras_destino.billetera_id"), nullable=True)
    monto = Column(Numeric(14, 2), nullable=False)
    estado = Column(Enum(DesembolsoEstado, name="desembolso_estado"), nullable=False, default=DesembolsoEstado.PROCESANDO)

    billetera = relationship("BilleteraDestino")


class PagoEstado(str):
    AL_DIA = "AL_DIA"
    EN_MORA = "EN_MORA"
...
class PlanPagosCobranza(Base):
    ...
    numero_cuota = Column(Integer, nullable=False)
    monto = Column(Numeric(14, 2), nullable=False)
    estado_pago = Column(Enum(PagoEstado, name="pago_estado"), nullable=False, default=PagoEstado.AL_DIA)
