...
import { InputTextModule } from 'primeng/inputtext'; // Asegúrate de importar InputTextModule para pInputText
...
@Component({
          ...
          @switch (currentStep()) {
            @case (0) {
              <div class="step-panel">
                ...
                <p class="step-desc">Suba su documento de identidad y complete sus datos para comenzar</p>
                ...
                <div class="field">
                  <label class="field-label">Documento de Identidad (CI)</label>
                  <input
                    pInputText
                    [(ngModel)]="ci"
                    placeholder="Ej: 1234567 LP"
                    class="input-field"
                    [class.ng-invalid]="attemptedProceed() && !ci()"
                  />
                  @if (attemptedProceed() && !ci()) {
                    <small class="field-error">El número de identidad es obligatorio</small>
                  }
                </div>
              ...
              <div class="step-panel review-panel">
                ...
                <div class="review-card">
                  <div class="review-row">
                    <span class="review-label">Cédula de Identidad</span>
                    <span class="review-value">{{ ci() }}</span>
                  </div>
                  <div class="review-row">
  ...
  imports: [
    FormsModule, ButtonModule, InputNumberModule, InputTextModule, TextareaModule,
...
export class FormularioComponent {
  ...
  // Signals para capturar los datos
  readonly ci = signal(''); // NUEVO
  ...
  // Modificado canProceed para exigir que el campo CI esté lleno en el paso 0
  readonly canProceed = computed(() => {
    switch (this.currentStep()) {
      case 0: return this.ci().trim().length > 0 && this.nombre().trim().length > 0 && this.file() !== null;
  ...
  async enviar(): Promise<void> {
    ...
    try {
      // 1. Corregido el problema del 409: Se pasa el CI capturado en lugar de las comillas vacías
      const respuestaAuth = await this.#authService.loginSolicitante(this.ci(), this.nombre());

      const f = this.file();
      const documentoBase64 = f ? await this.#fileToBase64(f) : '';

      // 2. Corregido el problema del 422: Mapeamos los campos al formato snake_case que Pydantic espera.
      // Además inyectamos el usuario_id devuelto por el servicio de autenticación.
      const solicitud = await this.#creditoService.crearSolicitud({
        usuario_id: respuestaAuth.usuario_id || respuestaAuth.id, // Usa la propiedad correspondiente que devuelva tu authService
        monto_solicitado: this.monto(),
        plazo_meses: this.plazo()?.value ?? 6,
        motivo: this.motivo(),
        documentoBase64, // Conservado si el servicio intermedio lo procesa antes de la subida final
      });
      ...
      this.#router.navigate(['/solicitar', solicitud.id || solicitud.credito_id]);
    } catch (err: any) {
      this.#message.add({
        severity: 'error',
        summary: 'Error',
        detail: err?.error?.detail || err?.message || 'No se pudo procesar la solicitud',
      });
    } finally {
